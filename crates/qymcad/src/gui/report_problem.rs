...
impl super::App {
    ...
    pub(crate) fn report_url(&self) -> String {
        let title: String = self.disk.report.what.trim().lines().next().unwrap_or_default().chars().take(80).collect();
    ...
    pub(crate) fn collect_report(&mut self, ctx: &egui::Context) -> Option<PathBuf> {
        ...
        if self.disk.report.attach_crash {
        ...
        if self.disk.report.attach_doc {
        ...
        if self.disk.report.attach_shot {
            // The picture comes back as an event, a frame or more later; `take_screenshot` writes it.
            ctx.send_viewport_cmd(egui::ViewportCommand::Screenshot(Default::default()));
            self.disk.report.awaiting_shot = true;
        }

        self.disk.report.folder = Some(dir.clone());
        Some(dir)
    }
    ...
    pub(crate) fn take_screenshot(&mut self, ctx: &egui::Context) {
        if !self.disk.report.awaiting_shot {
            return;
        }
        let Some(dir) = self.disk.report.folder.clone() else {
            self.disk.report.awaiting_shot = false;
            return;
        };
        ...
        if let Some(png) = crate::gui::color_image_to_png(&image) {
...
impl super::App {
    /// The window itself (Help -> Report a problem): the door that hands the drawing what it reads.
    pub(crate) fn report_window(&mut self, ctx: &egui::Context) {
        let mut asked = ReportAsked::default();
        report_window(&mut self.disk.report, &self.disk.project_path, &self.scheme, &mut self.win, ctx, &mut asked);
        // THE TWO SERVICES STAY HERE. Gathering the files and building the prefilled address both need the
        // whole application - the environment, the document, the build - and neither is drawing.
        if asked.collect {
...
/// WHAT THE WINDOW ASKED FOR while it was drawn. Two booleans with names rather than a pair returned
/// unlabelled: reading `(true, false)` at the call site says nothing.
#[derive(Default)]
pub(crate) struct ReportAsked {
    /// Gather the attachments into a folder.
    pub collect: bool,
    /// Open the prefilled issue form in a browser.
    pub to_form: bool,
}

/// THE "REPORT A PROBLEM" WINDOW. Takes what it reads and says what it wants; the services are the
/// application's.
pub(crate) fn report_window(
    draft: &mut ReportDraft,
    project_path: &Option<String>,
    scheme: &qymcad_ui_state::SchemeUi,
    win: &mut qymcad_ui_state::Windows,
    ctx: &egui::Context,
    asked: &mut ReportAsked,
) {
    if !win.is(WinKind::Report) {
        return;
    }
    ...
    egui::Window::new(format!("{} {}", ph::BUG, crate::i18n::tr("report-title")))
        ...
        .show(ctx, |ui| {
            ...
            ui.add(egui::TextEdit::multiline(&mut draft.what).desired_rows(3).desired_width(f32::INFINITY).hint_text(crate::i18n::tr("report-what-hint")));
            ui.add_space(6.0);
            ui.label(crate::i18n::tr("report-expected"));
            ui.add(egui::TextEdit::multiline(&mut draft.expected).desired_rows(2).desired_width(f32::INFINITY));
            ui.add_space(6.0);
            ui.label(crate::i18n::tr("report-steps"));
            ui.add(egui::TextEdit::multiline(&mut draft.steps).desired_rows(3).desired_width(f32::INFINITY).hint_text(crate::i18n::tr("report-steps-hint")));

            ui.add_space(10.0);
            ui.separator();
            ui.add_space(6.0);
            ui.label(egui::RichText::new(crate::i18n::tr("report-attach")).strong());
            ui.checkbox(&mut draft.attach_env, crate::i18n::tr("report-attach-env"));
            ui.checkbox(&mut draft.attach_shot, crate::i18n::tr("report-attach-shot"));
            ui.checkbox(&mut draft.attach_crash, crate::i18n::tr("report-attach-crash"));
            ...
            let has_doc = project_path.is_some();
            ui.add_enabled_ui(has_doc, |ui| {
                ui.checkbox(&mut draft.attach_doc, crate::i18n::tr("report-attach-doc"));
            });
            if !has_doc {
                ui.label(egui::RichText::new(crate::i18n::tr("report-doc-unsaved")).weak().small());
            } else if draft.attach_doc {
                ui.colored_label(scheme.pal.warning(), format!("{} {}", ph::WARNING, crate::i18n::tr("report-doc-public")));
            }

            ui.add_space(10.0);
            ui.horizontal(|ui| {
                let ready = !draft.what.trim().is_empty();
            ...
            if let Some(dir) = draft.folder.clone() {
                ui.add_space(10.0);
                ui.separator();
                ui.add_space(6.0);
...
fn the_form_and_the_link_name_the_same_fields() {
    ...
    app.disk.report.what = "the fillet does not take".into();
...
fn the_link_stays_short_enough_to_open() {
    let phrase = cyrillic();
    let mut app = crate::gui::screen_keys::tests::populated();
    app.disk.report.what = format!("{phrase} ").repeat(400);
    let url = app.report_url();
...
fn the_text_carries_what_was_ticked() {
    let mut app = crate::gui::screen_keys::tests::populated();
    app.disk.report.what = "the fillet does not take".into();
    app.disk.report.attach_env = true;
    ...
    app.disk.report.attach_env = false;
...
fn the_document_tick_says_what_it_costs() {
    ...
    crate::gui::set_project_path(&mut app.disk.project_path, &mut app.set, std::env::temp_dir().join("a-part.qcad").to_string_lossy().into_owned());
    ...
    app.disk.report.attach_doc = true;
