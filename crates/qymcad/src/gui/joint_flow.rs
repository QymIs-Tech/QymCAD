...
pub(in crate::gui) mod tests {
    ...
    fn a_joint_on_edges_goes_from_click_to_solution() {
        ...
        let edge_of = |app: &App, b: qymcad_core::model::Id| -> u32 {
            crate::gui::pick::body_edges_cached(&app.cache, &app.live, &app.regen, b).and_then(|e| e.ids.iter().copied().find(|&id| id != 0)).expect("the body has edges with persistent ids")
        };
        ...
        app.side.joint.pick_faces = true;
        app.side.joint.anchor_mode = 1; // THE EDGE ANCHOR — the very mode that was reported
        app.side.joint.new_kind = qymcad_core::feature::JointKind::Cylindrical;
        qymcad_assembly::joint_pick_edge_click(&mut app.joint_ctx(), ba, ea);
        assert!(app.side.joint.pick_first.is_some(), "the first click must fix anchor A");
        qymcad_assembly::joint_pick_edge_click(&mut app.joint_ctx(), bb, eb);
    ...
    fn picking_an_edge_anchor_asks_for_the_live_brep() {
        ...
        app.side.joint.pick_faces = true;
        app.side.joint.anchor_mode = 1; // the EDGE anchor
    ...
    fn a_sketch_on_a_face_still_shows_its_outline_after_opening_a_bundle() {
        ...
        app.live.tried_rev = None;
        crate::gui::commands::refresh_edges(&mut app.part_ctx());
        let after_open = crate::gui::sketching::sketch_ref_edges_2d(&app.cache, &app.tools.cmd, &app.live, &app.project, &app.regen, si).len();
    ...
    fn check_ring_sign(app: &mut App, ca: qymcad_core::model::Id, cb: qymcad_core::model::Id, kind: qymcad_core::feature::JointKind) {
        ...
        let amt = app.side.joint.giz_drag.expect("the drag is alive").amt;
        assert!(
            amt > 0.0,
    ...
    fn every_joint_kind_is_creatable_through_the_bar() {
        ...
        for kind in kinds {
            ...
            let (ba, bb) = (bodies[0], bodies[1]);
            let edge_of = |app: &App, b: qymcad_core::model::Id| -> u32 {
                crate::gui::pick::body_edges_cached(&app.cache, &app.live, &app.regen, b).and_then(|e| e.ids.iter().copied().find(|&id| id != 0)).expect("there are edges")
            };
            let (ea, eb) = (edge_of(&app, ba), edge_of(&app, bb));
            app.project.set_grounded(app.project.body_owner(ba).expect("the owner"), true);

            app.side.joint.pick_faces = true; // the Mate button
            app.side.joint.new_kind = kind; // the choice of kind in the bar
            app.side.joint.anchor_mode = 1;
            qymcad_assembly::joint_pick_edge_click(&mut app.joint_ctx(), ba, ea);
            qymcad_assembly::joint_pick_edge_click(&mut app.joint_ctx(), bb, eb);
    ...
    fn the_slide_arrow_of_a_pin_slot_points_where_the_part_will_go() {
        ...
        let handles = qymcad_ui_state::joint_giz_handles(&app.draw_ctx(), j).expect("the handles of the gizmo are there").handles;
        let dir = handles.iter().find(|h| h.slot == 1 && !h.ring).expect("a pin-slot has a slide handle").dir;
    ...
    fn the_geometry_is_up_before_the_first_face_anchor_is_picked() {
        ...
        assert_eq!(app.side.joint.anchor_mode, 0, "setup: the default anchor of a slider is the face");
    ...
    fn while_the_geometry_is_on_its_way_the_tool_does_not_take_picks() {
        ...
        app.side.joint.new_kind = JointKind::Slider;
        app.start_joint_pick();
        crate::gui::commands::refresh_edges(&mut app.part_ctx()); // a frame: the tool asked for the geometry and it went into the background
        ...
        assert!(
            qymcad_assembly::joint_pick_first_anchor_for_test(&mut app.joint_ctx()).is_none(),
        ...
        assert!(qymcad_assembly::joint_pick_active_for_test(&mut app.joint_ctx()), "the tool was dropped instead of waiting for the geometry");
    ...
    fn when_the_body_never_built_the_tool_says_so_instead_of_promising_a_moment() {
        ...
        app.start_joint_pick();
        qymcad_assembly::joint_pick_edge_click_for_test(&mut app.joint_ctx(), body, 1);
        assert!(qymcad_assembly::joint_pick_first_anchor_for_test(&mut app.joint_ctx()).is_none(), "an edge anchor was taken where there are no edges at all");
    ...
    fn pointing_at_an_edge_sets_the_anchor_axis_from_the_frame() {
        ...
        qymcad_assembly::joint_axis_pick_apply(&mut app.joint_ctx(), AnchorRef::EdgeMid(ba, e.id));
        ...
        assert!(app.side.joint.axis_pick.is_none(), "the point-at-the-axis tool did not release after the pick");

        // AND IT SHOWS IN THE MOVEMENT: the slider runs along the edge rather than along a guess.
        let axis = app.project.joint_slot_axis(jid, 1, qymcad_ui_state::current_ctx_id(&app.active_path, &app.project)).expect("the axis of travel");
        let want = e.dir;
