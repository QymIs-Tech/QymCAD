...
mod tests {
    ...
    fn assembly_with_selected_part() -> (App, egui::Context, u64) {
        ...
        let _ = ctx.run_ui(frame(Vec::new()), |c| app.viewport(c)); // lay out the canvas and learn its rectangle
        (app, ctx, comp)
    }
    ...
    fn dragging_the_component_gizmo_moves_the_part() {
        ...
        // moment.
        let _ = ctx.run_ui(frame(vec![egui::Event::PointerMoved(grab)]), |c| app.viewport(c));
        let _ = ctx.run_ui(frame(vec![press(grab, true)]), |c| app.viewport(c));
    ...
    fn dragging_away_from_the_handle_turns_the_camera_and_leaves_the_part_alone() {
        let (mut app, ctx, comp) = assembly_with_selected_part();
        let before = app.project.component_transform(comp);
        let yaw_before = app.viewing.cam.yaw;
        ...
        assert!((app.viewing.cam.yaw - yaw_before).abs() > 1e-6, "a drag away from the gizmo must turn the view, and the view did not stir");
    }
}
