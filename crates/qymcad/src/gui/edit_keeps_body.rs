...
mod tests {
    ...
    fn editing_a_fillet_leaves_the_part_on_screen() {
        for which in ["in the MIDDLE of the chain", "at the TOP of the chain"] {
            ...
            assert!(app.visible_mesh_count() > 0, "setup: before the edit the part is visible");
            app.chosen.sel = Sel::Feature(app.project.timeline.iter().position(|n| n.id == fid).expect("the node is there"));
            crate::gui::commands::start_feat_cmd_edit(&mut app.part_ctx(), fid);
            let during = app.visible_mesh_count();
            ...
            qymcad_ui_state::rebuild_if_dirty(&mut app.rebuild_ctx());
            assert!(app.visible_mesh_count() > 0, "after cancelling the edit {which} the part must come back");
        }
    }
    ...
    fn applying_an_edit_brings_the_part_back() {
        ...
        app.chosen.sel = Sel::Feature(app.project.timeline.iter().position(|n| n.id == first).expect("the node is there"));
        crate::gui::commands::start_feat_cmd_edit(&mut app.part_ctx(), first);
        ...
        assert!(app.visible_mesh_count() > 0, "after applying the edit the part must be on screen, not only after Rebuild Everything; status: {}", app.status.clone());
    }
}
