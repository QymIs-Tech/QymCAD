...
pub fn save(pal: &Palette) -> Result<PathBuf, String> {
    save_in(&dir().ok_or_else(|| qymcad_i18n::tr("io-no-config-dir"))?, pal)
}
...
pub fn delete(id: &str) -> Result<(), String> {
    delete_in(&dir().ok_or_else(|| qymcad_i18n::tr("io-no-config-dir"))?, id)
}

/// The same, but in a GIVEN directory.
pub fn delete_in(d: &std::path::Path, id: &str) -> Result<(), String> {
    let path = path_for_id_in(d, id).ok_or_else(|| qymcad_i18n::tr1("scheme-file-missing", "name", id))?;
