...
///
/// EVERY CRATE, NOT ONE. This walked the `src` of the crate it happened to live in, which was the
/// application - so when the interface moved out into its own crates the guard kept passing while
/// seeing less and less of what it guards. Measured when it was widened: 14 crates instead of 1, and
/// one live defect behind the blind spot - a raw arrow in the rebind message of `model/regen.rs`,
/// which reaches the status line through `Rebind::what` and is drawn there as a square. Its twin
/// message in `model.rs` had the ASCII `->` all along, so the two copies had already drifted.
///
/// The base is the REPOSITORY, not this crate, and that is the point: a guard addressed to one crate
/// goes blind silently the next time code moves.
#[test]
fn the_ui_code_holds_words_not_symbols() {
    ...
    while let Some(dir) = stack.pop() {
        for e in std::fs::read_dir(&dir).expect("the sources read").flatten() {
            ...
            let code = qymcad_i18n::ratchet::working_part(&text);
