...
- **Модуль ЧПУ (CAM) не работает.** В настройках он включается галкой «Вкладка „Обработка“ (CAM)», но
  ...
  стабильной альфе.
- **У сборки под macOS нет подписи Apple.** Система помечает скачанную программу карантином и не даёт
  её открыть — говорит, что она повреждена, хотя это не так. Метка снимается один раз, одной командой;
  она расписана по шагам в текстах внутри архива. Только Apple Silicon, сборки под Intel нет.
